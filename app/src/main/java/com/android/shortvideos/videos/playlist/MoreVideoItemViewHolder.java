...
import com.android.shortvideos.videos.common.MediaUrl;
...
public class MoreVideoItemViewHolder extends RecyclerView.ViewHolder implements ToroPlayer {
  ...
  void bind(MoreVideosAdapter adapter, FbVideo item, List<Object> payloads) {
    if (item != null) {
      ...
      String mediaURL = url.getUri().toString();
      HttpProxyCacheServer proxyServer = new HttpProxyCacheServer.Builder(userIcon.getContext()).maxCacheSize(1024 * 1024 * 1024).build();

      String proxyURL = proxyServer.getProxyUrl(mediaURL);
      mediaUri = Uri.parse(proxyURL);
